...
<div class="row"> 
    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
        <div class="page-header">
            <h2 class="pageheader-title">Edit Berita</h2>
            <p class="pageheader-text">Halaman untuk edit berita</p>
        ...
        <h3 class="text-center">Edit Berita</h3>
            ...
            @method('PATCH')
                <div class="form-group inp">
                    ...
                    <div class="input">
                        <input type="text" name="judul" id="judul" autocomplete="off" class="inp" value="{{$berita->judul}}" required> 
                    </div> 
                </div>
                <div class="form-group inp">
                    <label for="judul">Tanggal<span class="text-danger">*</span></label>
                    <div class="input">
                        <input id="date" type="text" name="date" id="judul" autocomplete="off" class="inp" value="{{$berita->date}}" required>
                    </div> 
                </div>
                    ...
                    <select name="kategori" class="form-control" required>
                        <option value="">--Pilih Kategori--</option required>
                            @foreach($kategori as $key=>$data)
                                <option value='{{$data->id}}' {{(($data->id==$berita->kategori)? 'selected' : '')}}>{{$data->nama}}</option>
                            @endforeach
                    </select>
                </div>
                @php 
                ...
                <div class="form-group">
                    ...
                    <textarea id="summernote" name="deskripsi" >{{$berita->deskripsi}}</textarea>
                </div>
                <div class="form-group">
                    ...
                    <input id="thumbnail" class="form-control" type="text" name="photo" value="{{$berita->photo}}" required>
                    </div>
